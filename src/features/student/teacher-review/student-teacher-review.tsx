  ...
  return (
    <main className="mx-auto w-full max-w-2xl flex-1 px-4 pb-24 pt-6 sm:px-6 animate-in fade-in duration-300">
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-primary via-orange-400 to-amber-400 p-6 text-white mb-5 shadow-lg shadow-primary/20">
        ...
        <span className="inline-flex items-center gap-1.5 mt-3 rounded-full bg-white/20 px-3 py-1 text-xs font-bold backdrop-blur-sm">
          Đợt giữa khóa · mở đến 30/06/2026 · mỗi Thầy/Cô 1 lần
        </span>
      </div>

      {/* Chọn giáo viên */}
      <p className="text-sm font-bold text-slate-900 mb-2">Em muốn đánh giá ai?</p>
        ...
        {teachers.map((t) => {
          ...
          return (
            <button
              ...
              className={cn('flex items-center gap-3 p-3.5 rounded-2xl border-2 text-left transition-all cursor-pointer', active ? 'border-primary bg-primary/5 shadow-sm' : 'border-border/60 bg-white hover:border-slate-300')}
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-sm text-slate-500 font-medium truncate">{t.subject}</p>
              </div>
              {reviewed ? (
                ...
                <span className="size-5 grid place-items-center rounded-full bg-primary text-white text-xs shrink-0">✓</span>
              ) : null}
            </button>
          )
        })}
      ...
      {success ? (
        <Card className="border-success/20 bg-success/5 rounded-3xl p-8 text-center flex flex-col items-center gap-3">
          ...
          <h2 className="text-xl font-black text-slate-900">Tuyệt vời, cảm ơn em!</h2>
          <p className="text-sm text-slate-600 font-semibold max-w-md leading-relaxed">Lời đánh giá của em dành cho <strong>{selected}</strong> đã được gửi (bí mật) rồi nhé!</p>
        </Card>
      ) : isLocked ? (
        <Card className="border-border/60 rounded-3xl p-8 text-center flex flex-col items-center gap-3 bg-slate-50">
          <span className="grid size-16 place-items-center bg-slate-100 text-slate-400 rounded-full"><Lock className="size-8" /></span>
          <h2 className="text-xl font-bold text-slate-900">Em đã đánh giá Thầy/Cô này rồi</h2>
        ...
        <>
          ...
          <div className="flex items-center gap-3 mb-3">
            ...
            <span className="text-sm font-bold text-slate-500 shrink-0">{answered}/{SCHEMA.length}</span>
          </div>
              ...
              return (
                <div key={f.id} className={cn('rounded-3xl border-2 p-5', pal.bg)}>
                  <div className="flex items-start gap-2.5 mb-3.5">
                    ...
                    <p className="text-base font-bold text-slate-900 leading-snug">{f.question}{f.required && <span className="text-destructive"> *</span>}</p>
                  </div>
                      ...
                      {f.options.map((o) => {
                        ...
                        return (
                          <button key={o.value} onClick={() => set(f.id, o.value)}
                            className={cn('flex flex-col items-center justify-center py-3 rounded-2xl border-2 bg-white transition-all cursor-pointer', sel ? 'border-primary ring-2 ring-primary/25 scale-105 shadow-sm' : 'border-transparent hover:scale-105')}>
                            <span className="text-4xl sm:text-5xl">{o.emoji}</span>
                            <span className={cn('text-xs font-bold mt-1', sel ? 'text-primary' : 'text-slate-500')}>{o.label}</span>
                          </button>
                        )
                      })}
                    </div>
                  ...
                  {f.type === 'star' && (
                    <div className="flex flex-wrap items-center gap-3">
                      ...
                      {formData[f.id] > 0 && <span className="text-sm font-black text-amber-600 bg-white px-3 py-1.5 rounded-full shadow-2xs">{STAR_LABEL[formData[f.id]]}</span>}
                    </div>
                  )}
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center gap-3">
                        ...
                        <input type="range" min={1} max={10} value={formData[f.id] || 5} onChange={(e) => set(f.id, parseInt(e.target.value))}
                          className="w-full h-3 bg-white rounded-full appearance-none cursor-pointer accent-primary" style={{ minHeight: 44 }} />
                        <span className="text-2xl">😍</span>
                        <span className="size-12 grid place-items-center bg-primary text-white rounded-full text-lg font-black shrink-0">{formData[f.id] || 5}</span>
                      </div>
                      <div className="flex justify-between text-xs text-slate-500 font-bold uppercase px-9">
                  ...
                  {f.type === 'text' && (
                    <textarea value={formData[f.id] || ''} onChange={(e) => set(f.id, e.target.value)} rows={3}
                      ...
                      className="w-full p-3.5 rounded-2xl border-2 border-white bg-white focus:outline-none focus:ring-2 focus:ring-primary/25 text-sm font-medium" />
                  )}
                </div>
              )
            })}
          ...
          <button onClick={() => { if (validate()) setConfirm(true) }}
            className="mt-5 w-full h-14 rounded-full bg-gradient-to-r from-primary to-amber-400 text-white text-base font-black hover:opacity-95 active:scale-[0.99] cursor-pointer inline-flex items-center justify-center gap-2 shadow-lg shadow-primary/25">
      ...
      {confirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-xs p-4 animate-in fade-in duration-200">
          <div className="bg-white border border-slate-200 p-6 w-full max-w-sm rounded-3xl shadow-xl text-center space-y-4">
            <span className="text-5xl block">💖</span>
            <h3 className="text-base font-bold text-slate-900">Gửi đánh giá nhé?</h3>
            ...
            <div className="flex justify-center gap-3 pt-1">
              <button onClick={() => setConfirm(false)} className="h-11 rounded-full border border-border bg-card px-5 text-sm font-bold text-slate-700 hover:bg-slate-50 cursor-pointer min-w-[110px]">Để sau</button>
              <button onClick={submit} disabled={submitting} className="h-11 rounded-full bg-primary px-6 text-sm font-black text-white hover:opacity-90 cursor-pointer border-none min-w-[110px]">{submitting ? 'Đang gửi...' : 'Gửi ngay!'}</button>
            </div>
          </div>
        </div>
      )}
