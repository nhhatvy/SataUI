        ...
        <div className="lg:col-span-2 space-y-6">
              ...
              <Card className="border-caution/30 bg-caution/[0.03] rounded-2xl shadow-none">
                <CardContent className="p-5 space-y-4">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <span className="text-sm text-muted-foreground font-semibold uppercase tracking-wider">{tuition?.period}</span>
          ...
          <section className="space-y-3">
            <h2 className="text-base font-bold text-foreground uppercase tracking-wider flex items-center gap-2">
              <ReceiptText className="size-4 text-muted-foreground" /> Phiếu thu đã xác nhận
            </h2>
            <Card className="border-border/60 rounded-2xl shadow-none overflow-hidden">
              <CardContent className="p-0 divide-y divide-border">
                {transactions.filter((t) => t.status === 'paid').length === 0 ? (
                  <p className="p-6 text-center text-sm text-muted-foreground font-semibold">Chưa có phiếu thu nào được xác nhận.</p>
                ) : (
                  transactions
      ...
      <Dialog open={showQR} onOpenChange={(o: boolean) => !o && setShowQR(false)}>
        <DialogContent className="max-w-sm p-0 overflow-hidden rounded-2xl">
          <div className="p-6 text-center">
            <DialogTitle className="text-base font-bold text-foreground">Thanh toán VietQR</DialogTitle>
            <p className="text-sm text-muted-foreground font-medium mt-1">Quét mã bằng ứng dụng ngân hàng</p>
            {success ? (
              <div className="py-12 flex flex-col items-center gap-3">
                ...
                <p className="text-sm font-bold text-foreground">Đã ghi nhận thanh toán!</p>
                <p className="text-sm text-muted-foreground font-medium">Chờ kế toán xác nhận thực thu.</p>
              </div>
            ) : (
              <>
                <div className="my-5 aspect-square max-w-[13rem] mx-auto border border-dashed border-border p-3 rounded-2xl bg-muted flex items-center justify-center">
                  <div className="size-40 bg-foreground/10 rounded-lg grid place-items-center text-sm font-bold text-muted-foreground">[ Mã QR mẫu ]</div>
                </div>
                <div className="text-left text-sm bg-muted/60 rounded-xl p-3 space-y-1 border border-border/40 font-semibold mb-4">
