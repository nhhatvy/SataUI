...
import { Menu, UserRound, LogOut, Check, Palette } from 'lucide-react'
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    active ? 'bg-accent-soft text-accent font-bold' : 'text-muted-foreground hover:bg-muted font-medium'
                  )}
                >
          ...
          <div className="mt-4 px-3">
            ...
            <button
              ...
              className={cn(
                ...
                !isStudentMode ? 'bg-parent/10 text-parent' : 'text-muted-foreground hover:bg-muted'
              )}
            >
        ...
        <div className="border-t border-border/50 p-3 shrink-0">
          <div className="flex items-center gap-2.5 px-2 py-2">
            ...
            <div className="min-w-0">
              <p className="text-sm font-bold text-foreground truncate">{user?.name || 'Tài khoản'}</p>
              <p className="text-xs text-muted-foreground font-medium truncate">{isStudentMode ? `Học sinh: ${child?.shortName}` : 'Phụ huynh'}</p>
            </div>
          </div>
          <button
            onClick={() => go('/settings/appearance')}
            className="mt-1 flex w-full items-center gap-2.5 rounded-xl px-3 py-2.5 text-sm font-bold text-muted-foreground hover:bg-muted transition-colors"
          >
            <Palette className="size-4.5" /> Giao diện
          </button>
          <button
            onClick={handleLogout}
            className="flex w-full items-center gap-2.5 rounded-xl px-3 py-2.5 text-sm font-bold text-destructive hover:bg-destructive/5 transition-colors"
          >
