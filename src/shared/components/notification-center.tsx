  ...
  const railBtn = (key: 'all' | NotiCategory, label: string, count: number, Icon?: LucideIcon) => (
    <button
      ...
      className={cn(
        ...
        active === key ? 'bg-accent-soft text-accent' : 'text-muted-foreground hover:bg-muted'
      )}
    >
  ...
  const renderItem = (n: NotificationItem) => {
    ...
    return (
      <button
        key={n.id}
        onClick={() => open(n)}
        className={cn('flex w-full items-start gap-3 p-3.5 sm:p-4 text-left transition-colors hover:bg-muted cursor-pointer relative', n.unread && 'bg-primary/[0.02]')}
      >
        {n.unread && <span className="absolute left-0 top-0 bottom-0 w-1 bg-primary" />}
        <span className={cn('mt-0.5 grid size-9 sm:size-10 shrink-0 place-items-center rounded-xl border', m?.cls ?? 'text-muted-foreground bg-muted border-border')}>
          <Icon className="size-4.5 sm:size-5" />
        </span>
        <div className="min-w-0 flex-1">
          {/* Hàng 1: tiêu đề (1 dòng) + tên con (nếu có) */}
          <div className="flex items-center gap-2">
            <span className={cn('text-sm text-foreground truncate min-w-0 flex-1', n.unread ? 'font-bold' : 'font-semibold')}>{n.title}</span>
            {showChild && (
          ...
          {/* Mô tả: tối đa 2 dòng */}
          <p className="text-sm text-muted-foreground font-medium leading-snug mt-0.5 line-clamp-2">{n.description}</p>
          ...
          {filtered.length === 0 ? (
            <Card className="border-border/60 shadow-none rounded-2xl p-12 text-center">
              <CardContent className="flex flex-col items-center gap-3 p-0">
                <span className="size-14 rounded-full bg-muted grid place-items-center text-muted-foreground"><Bell className="size-7" /></span>
                <p className="text-sm font-bold text-muted-foreground">Không có thông báo</p>
                <p className="text-sm text-muted-foreground">Chưa có cập nhật nào trong mục này.</p>
              </CardContent>
            </Card>
          ) : (
            <div className="space-y-5">
              {unread.length > 0 && (
                <section>
                  <h3 className="text-sm font-bold text-foreground uppercase tracking-wider mb-2">Chưa đọc ({unread.length})</h3>
                  <Card className="border-border/60 rounded-2xl shadow-none overflow-hidden py-0">
                    <CardContent className="p-0 divide-y divide-border">{unread.map(renderItem)}</CardContent>
                  </Card>
                </section>
              )}
              {read.length > 0 && (
                <section>
                  <h3 className="text-sm font-bold text-muted-foreground uppercase tracking-wider mb-2">Đã đọc</h3>
                  <Card className="border-border/60 rounded-2xl shadow-none overflow-hidden py-0">
                    <CardContent className="p-0 divide-y divide-border">{read.map(renderItem)}</CardContent>
                  </Card>
                </section>
              )}
            </div>
